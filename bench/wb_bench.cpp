...
// from the results; the return is folded into `sink` (printed at the end) so the
...
// untimed pass would literally double the wall clock for no benefit — their buffers
// are far too large for cache warming to matter, and no one-off arena cost applies.
template <typename F>
